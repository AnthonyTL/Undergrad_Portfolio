...
def fits(time):
    ...
    for n in nums:
        ...
        fig.text(0.5, 0.04, 'Frequency (MHz)', ha='center', fontsize=25)
        ...
        ax[1].axvline(xdata[stop1], color='fuchsia')  # plots the expected frequency
        ax[1].axvline(xdata[start2], color='fuchsia')  # plots the expected frequency
        ...
        print('-------------------------------------------')
        max_ticks = 4
        ax[1].xaxis.set_major_locator(MaxNLocator(max_ticks))
    ...
    fig, ax1 = plt.subplots(figsize=(6.4 * x, 4.8 * x))
    ...
    tick_positions = np.arange(len(nums))

    # Set the tick labels for the specific ticks
    tick_labels = ['' if i % 2 != 0 else PDT[i] for i in range(len(nums))]

    # Set the tick positions and labels for the first x-axis
    ax1.set_xticks(tick_positions)
    ax1.set_xticklabels(tick_labels)
    ax1.set_xlabel('Index')
    ax1.set_xlabel('Time (PDT)')
    ax1.set_ylabel('Hydrogen Column Density (cm^-2)')
    # Create a second x-axis
    ax2 = ax1.twiny()
    # Set the second x-axis ticks and labels
    ax2.set_xlim(ax1.get_xlim())
    tick_positions2 = np.arange(len(nums))
    # Set the tick labels for the specific ticks
    tick_labels2 = ['' if i % 2 != 0 else lon[i] for i in range(len(nums))]
    # Set the tick positions and labels for the first x-axis
    ax2.set_xticks(tick_positions2)
    ax2.set_xticklabels(tick_labels2)
    ax2.set_xlabel('Galatic Longetude (Degrees)')

    # fig.text(0.098, 0.845, '1e21', ha='center', fontsize=18)
    fig.text(0.098, 0.875, '1e21', ha='center', fontsize=18)

    max_ticks=4
    ax1.yaxis.set_major_locator(MaxNLocator(max_ticks))
    plt.savefig('N prgression {}.pdf'.format(time))
    plt.show()


def skymap():
    x = 2
    plt.figure(figsize=(6.4 * x, 4.8 * x))
